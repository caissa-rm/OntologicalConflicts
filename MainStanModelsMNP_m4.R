...
N <- nrow(dr_mnp_s)            # total number of responses
...
dat4 <- list(
  ...
  id = dr_mnp_s$ID,              # vector of person IDs
  N_id = length(unique(dr_mnp_s$ID)),    # number of people (raters)
  item1 = as.integer(dr_mnp_s$IDitem1),    # vector of question rank1 IDs
  item2 = as.integer(dr_mnp_s$IDitem2),    # vector of question rank2 IDs
  y = dr_mnp_s$y               # vector of responses
)
...
mr4 <- readRDS("mr4_fit.rds") #to read in already-fit stanfit output
