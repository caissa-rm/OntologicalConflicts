...
denschart3 <- function (x, labels = NULL, groups = NULL, gdata = NULL, cex = par("cex"), 
    ...
    main = NULL, xlab = NULL, ylab = NULL, height=0.7 , border=NA, adjust=1, ...) 
  {
    ...
    for ( i in 1:n ) {
        ...
        polygon( a$x , a$y , col=color , border=NA ) 
        Cuts <- HPDI( x[[i]] , HPDI)
...
Stan_traces <- function(m = m1){
    print(traceplot(m, pars="b0", inc_warmup=T))
    for ( z1 in 1:J ){
        print(traceplot(m, pars=paste("bIndiv[", z1, "]", sep=""), inc_warmup=T ))
    }
    for ( z2 in 1:(K) ){
        print(traceplot(m, pars=paste("beta[", z2, "]", sep=""), inc_warmup=T ))
    }
    for ( z3 in 1:K ){
        print(traceplot(m, pars=paste("gamma[", z3, "]", sep=""), inc_warmup=T ))
    }
    print(traceplot(m, pars="sigma_beta", inc_warmup=T))
    print(traceplot(m, pars="sigma_gamma", inc_warmup=T))
